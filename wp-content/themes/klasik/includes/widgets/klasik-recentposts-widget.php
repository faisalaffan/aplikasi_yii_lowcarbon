            ...
            $before_widget = str_replace('class="', 'class="'. $customclass . ' ', $before_widget);
					...
					$output .='<div class="klasik-recentpost-widget '.$customclass.'">';
						$output .='<div class="row">';
							...
							$tpl ='<div id="recentpost-%%ID%%" class="%%CLASS%%">';
								...
								$tpl .= '</div>';
							$tpl .= '</div>';
							$tpl = apply_filters( 'klasik_recentpost_item_template', $tpl );
							...
							if ($wp_query->have_posts()) : 
								...
								$cf_thumb = get_the_post_thumbnail($post->ID, 'widget-post', array('class' => 'alignleft'));
