...
function cpotheme_admin_welcome_notice(){	
	if(current_user_can('manage_options')){
		...
		if(current_user_can('manage_options') && $welcome_dismissed != 'dismissed' && $display){
			...
			$welcome_url = '<a href="'.esc_url(admin_url('themes.php?page=cpotheme-welcome')).'">'.__('quickstart guide', 'intuition').'</a>';
			...
			echo '<a href="'.add_query_arg('ctdismiss', CPOTHEME_ID.'_wizard').'" class="cpotheme-notice-dismiss">'.__('Dismiss This Notice', 'intuition').'</a>';
			echo '<p>'.sprintf(esc_html__('%s is ready! It is highly recommended that you install the %s companion plugin and then check the quickstart guide to see how it all works.', 'intuition'), esc_attr(CPOTHEME_NAME), $plugin_url, $welcome_url).'</p>';
			echo '<p><a href="'.esc_url(admin_url('themes.php?page=cpotheme-welcome')).'" class="button button-primary" style="text-decoration: none;">'.sprintf(__('Start Using %s', 'intuition'), esc_attr(CPOTHEME_NAME)).'</a></p>';
			echo '</div>';
			echo '</div>';
		}
	}
