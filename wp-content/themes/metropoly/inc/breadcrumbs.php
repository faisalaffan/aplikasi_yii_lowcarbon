...
* @return void
*/
function metropoly_breadcrumb_trail( $args = array() ) {
if ( function_exists( 'is_bbpress' ) && is_bbpress() )
$breadcrumb = new metropoly_bbPress_Breadcrumb_Trail( $args );
else
$breadcrumb = new metropoly_Breadcrumb_Trail( $args );
...
* @since 0.6.0
*/
class metropoly_Breadcrumb_Trail {
/**
...
$labels = array(
'browse' => __( 'Browse:', 'metropoly' ),
'home' => __( 'Home', 'metropoly' ),
'error_404' => __( '404 Not Found', 'metropoly' ),
'archives' => __( 'Archives', 'metropoly' ),
/* Translators: %s is the search query. The HTML entities are opening and closing curly quotes. */
'search' => __( 'Search results for &#8220;%s&#8221;', 'metropoly' ),
/* Translators: %s is the page number. */
'paged' => __( 'Page %s', 'metropoly' ),
/* Translators: Minute archive title. %s is the minute time format. */
'archive_minute' => __( 'Minute %s', 'metropoly' ),
/* Translators: Weekly archive title. %s is the week date format. */
'archive_week' => __( 'Week %s', 'metropoly' ),
...
public function do_singular_items() {
 ...
 $show_categories        = metropoly_option('breadcrumb_show_categories','yes');
 $show_post_type_archive = metropoly_option('breadcrumb_show_post_type_archive','yes');
...
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute_hour'], get_the_time( _x( 'g:i a', 'minute and hour archives time format', 'metropoly' ) ) );
}
/**
...
/* Add the minute item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_minute'], get_the_time( _x( 'i', 'minute archives time format', 'metropoly' ) ) );
}
/**
...
/* Add the hour item. */
if ( true === $this->args['show_title'] )
$this->items[] = sprintf( $this->args['labels']['archive_hour'], get_the_time( _x( 'g a', 'hour archives time format', 'metropoly' ) ) );
}
/**
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'metropoly' ) ) );
$month = sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'metropoly' ) ) );
$day = sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'metropoly' ) ) );
...
$year = sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'metropoly' ) ) );
$week = sprintf( $this->args['labels']['archive_week'], get_the_time( _x( 'W', 'weekly archives date format', 'metropoly' ) ) );
/* Add the year item. */
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y' ) ) . '" title="' . esc_attr( $year ) . '">' . $year . '</a>';
/* Add the week item. */
if ( is_paged() )
...
if ( '%year%' == $tag )
$this->items[] = '<a href="' . get_year_link( get_the_time( 'Y', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_year'], get_the_time( _x( 'Y', 'yearly archives date format', 'metropoly' ) ) ) . '</a>';
/* If using the %monthnum% tag, add a link to the monthly archive. */
elseif ( '%monthnum%' == $tag )
$this->items[] = '<a href="' . get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_month'], get_the_time( _x( 'F', 'monthly archives date format', 'metropoly' ) ) ) . '</a>';
/* If using the %day% tag, add a link to the daily archive. */
elseif ( '%day%' == $tag )
$this->items[] = '<a href="' . get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) . '">' . sprintf( $this->args['labels']['archive_day'], get_the_time( _x( 'j', 'daily archives date format', 'metropoly' ) ) ) . '</a>';
...
* @access public
*/
class metropoly_bbPress_Breadcrumb_Trail extends metropoly_Breadcrumb_Trail {
/**
